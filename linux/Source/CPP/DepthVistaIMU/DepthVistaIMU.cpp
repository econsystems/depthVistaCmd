...
void* GetIMUValueThread(void* lpParameter)
{
    ...
    //HID command
    if(GetIMUValue(&IMUDataReadyEvent , lIMUOutput))
    {
...
{
	IMUDATAOUTPUT_TypeDef* lIMUOutput = static_cast<IMUDATAOUTPUT_TypeDef*>(lpParameter);
	UINT16 lIMUValID = 0, lNumofValues = 0;
...
DWORD WINAPI UpdateIMUValueThread(void* lpParameter)
{
		...
		(lIMUOutput->imu_value_id <= glIMUInput.imu_num_of_values))); )
	{
		if (glIMUInput.imu_update_mode != IMU_CONT_UPDT_DIS)
		{
			WaitForSingleObject(IMUDataReadyEvent, INFINITE);
		}
...
int IMU_Sample::Init()
{
	...
	if (uStatus)
	{
		...
		else
		{
			...
			//std::cout << "Debug : Set IMU Config success " << glIMU_Interval << "\r\n";
	...
	DWORD lThreadID, lAPIStatus;
	HANDLE lAPIHandle, lPrintHandle;
    BOOL lAPIThreadStatus = false, lPrintThreadStatus = false;
...
#ifdef _WIN32
    IMUDataReadyEvent = CreateEvent(NULL, false, false, NULL);

	lAPIHandle = CreateThread(0, 0, GetIMUValueThread, (void*)lIMUOutput, 0, &lThreadID);
	lPrintHandle = CreateThread(0, 0, UpdateIMUValueThread, (void*)lIMUOutput, 0, &lThreadID);

	for (; (!_kbhit() && (!lAPIThreadStatus || !lPrintThreadStatus) && (glIMUInput.imu_update_mode != IMU_CONT_UPDT_DIS));)
	{
		//Wait untill the IMU data is received from the camera
		if (!lAPIThreadStatus)
		{
			lAPIStatus = WaitForSingleObject(lAPIHandle, 0);
			if (lAPIStatus == WAIT_FAILED)
			{
				std::cout << "API thread Error!\r\n";
				break;
			}
			else if (lAPIStatus == WAIT_OBJECT_0)
			{
				lAPIThreadStatus = TRUE;
			}
		}
...
#ifdef  _WIN32
	WaitForSingleObject(lAPIHandle, 1000);
	TerminateThread(lAPIHandle, 0);
	CloseHandle(lAPIHandle);

	WaitForSingleObject(lPrintHandle, 1000);
	TerminateThread(lPrintHandle, 0);
	CloseHandle(lPrintHandle);
	CloseHandle(IMUDataReadyEvent);
	IMUDataReadyEvent = NULL;
    glIMUAbortThread = false;
#elif __linux__
